...
// passer des references
std::tuple< arma::mat, arma::vec, arma::vec> Kriging::apply(const arma::vec y, const arma::mat X, const arma::vec theta, const arma::mat Xtest) {
  ...
  return std::make_tuple(std::move(gamma), std::move(theta), std::move(Ytest));
}